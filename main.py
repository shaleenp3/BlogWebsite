...
app.config.update(
    ...
    MAIL_PORT='465',
    MAIL_USE_SSL=True,
    MAIL_USERNAME=params['gmail_userid'],
...
def contact():
    if request.method == 'POST':
        ...
        entry = Contacts(name=name,email=email,phn_num=phone,date=datetime.now(),msg=message)
        db.session.add(entry)
        db.session.commit()
        mail.send_message('New message from Blog! --' + name,sender=email, recipients=[params['gmail_userid']], body = message + "\n" + phone)
